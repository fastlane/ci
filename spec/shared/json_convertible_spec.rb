...
module FastlaneCI
  describe JSONConvertible do
    ...
    it "Allows to object dictionary with custom property mapping" do
      allow(
        MockJSONConvertible
      ).to receive(:attribute_key_name_map).and_return({ :@one_attribute => "json_one_attribute" })
      json_from_mock_object = mock_object.to_object_dictionary
    ...
    it "Allows to perform custom mappings of the values for a given attribute when converting to JSON" do
      allow(
        MockJSONConvertible
      ).to receive(:attribute_name_to_json_proc_map).and_return(
        { :@other_attribute => proc { |timestamp| timestamp.strftime("%G-W%V-%uT%R%:z") } }
      )
      json_from_mock_object = mock_object.to_object_dictionary
      expect(
        json_from_mock_object
      ).to eql({ "one_attribute" => "Hello", "other_attribute" => Time.at(0).strftime("%G-W%V-%uT%R%:z") })
    end

    it "Allows to perform custom property mapping and value mapping at the same time" do
      allow(
        MockJSONConvertible
      ).to receive(:attribute_key_name_map).and_return(
        { :@one_attribute => "json_one_attribute", :@other_attribute => "json_other_attribute" }
      )
      allow(
        MockJSONConvertible
      ).to receive(:attribute_name_to_json_proc_map).and_return(
      ...
      expect(
        json_from_mock_object
      ).to eql({ "json_one_attribute" => "Hello", "json_other_attribute" => Time.at(0).strftime("%G-W%V-%uT%R%:z") })
    end
    ...
    it "Allows to decode from a dictionary object using custom value mapping" do
      allow(
        MockJSONConvertible
      ).to receive(:json_to_attribute_name_proc_map).and_return(
        { :@other_attribute => proc { |timestamp| Time.at(timestamp) } }
      )
    ...
    it "Allows to decode from a dictionary object when it has custom attributes" do
      allow(
        MockParentJSONConvertible
      ).to receive(:attribute_to_type_map).and_return({ :@mock_json_attribute => MockJSONConvertible })
    ...
    it "Allows to decode from a dictionary object when it has an array typed attribute" do
      allow(
        MockArrayJSONConvertible
      ).to receive(:attribute_to_type_map).and_return({ :@mock_json_array_attribute => MockJSONConvertible })
      dictionary_object = {
        "mock_json_array_attribute" => [
    ...
    it "Allows to decode objects with nested array properties" do
      mock_object = MockAttributeArrayJSONConvertible.new(
        one_attribute: "World", other_attribute: Time.at(10),
        array_attribute: [MockJSONConvertible.new(one_attribute: "Inner World", other_attribute: Time.at(100))]
      )
      ...
      expect(
        ...
        {
          "one_attribute" => "World",
          "other_attribute" => Time.at(10),
          "array_attribute" => [
            {
              "one_attribute" => "Inner World",
              "other_attribute" => Time.at(100)
            }
          ]
        }
      )
