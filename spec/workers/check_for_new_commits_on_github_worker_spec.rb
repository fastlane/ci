...
describe FastlaneCI::CheckForNewCommitsOnGithubWorker do
  ...
  let(:master_prev_run_sha) { "acdacfe6-af6e-4190-a627-f85ea4cfa769" }
  ...
  let(:build_doubles) do
    [
      double("build_0", branch: "master", sha: master_prev_run_sha),
      double("build_1", branch: "master", sha: "84793bab-80b8-4141-9398-804a72c73851"),
      double("build_2", branch: "patch-1", sha: "76023ce3-aaf5-4b31-922f-78086a91095b"),
      double("build_3", branch: "patch-1", sha: "47ef6f6a-c49d-467b-9c86-620ff1aecadd"),
  ...
  let(:branch_name_to_commits_doubles) do
    {
      ...
      "master" => [
        double("commit_0", sha: master_prev_run_sha),
  ...
  describe "#filter_branch_name_to_commits_mapping" do
    context "`master` branch only (one unrun sha, one previously run sha)" do
      ...
      it "properly filters out commits where the sha exists in a build" do
        ...
        expect(filtered_commit_shas).not_to include(master_prev_run_sha)
    ...
    context "`patch-1` branch only (two unrun shas, no previously run shas)" do
      ...
      it "properly filters out commits where the sha exists in a build" do
        ...
        # Sanity check.
        expect(filtered_commit_shas).not_to include(master_prev_run_sha, master_unrun_sha, patch_2_prev_run_sha_one, patch_2_prev_run_sha_two)
      end
    end
    ...
    context "`patch-2` branch only (no unrun shas, two previously run shas)" do
      ...
      it "properly filters out commits where the sha exists in a build" do
        ...
        # Sanity check.
        expect(filtered_commit_shas).not_to include(master_prev_run_sha, master_unrun_sha, patch_1_unrun_sha_one, patch_1_unrun_sha_two)
      end
    end
    ...
    context "All three branches (three unrun shas, three previously run shas)" do
      ...
      it "properly filters out commits where the sha exists in a build" do
        ...
        expect(filtered_commit_shas).not_to include(master_prev_run_sha, patch_2_prev_run_sha_one, patch_2_prev_run_sha_two)
      end
    end
  end
end
